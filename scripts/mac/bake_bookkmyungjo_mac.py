"""
BookkMyungjo Vollkorn 컨텍스트 적용 (Mac 버전)
Light → NotoSansJP-Light (Vollkorn Regular/Italic)
Bold  → NotoSansSC-SemiBold (Vollkorn SemiBold/SemiBoldItalic)
"""
...
SC_SEMIBOLD_FONT_PID = -1629860644874142470
SC_SEMIBOLD_TEX_PID  =  8010781936477224186

VOLLKORN_BASE_PID       =  6994376558487935071
VOLLKORN_SEMIBOLD_PID   = -3676257516557452952
...
def build_and_save(font_pid, tex_pid, packed, atlas, label):
    ...
    for obj in env.objects:
        if obj.type.name == 'MonoBehaviour' and obj.path_id == font_pid:
            ...
            d.save()
            print(f'  {label}: {len(new_chars)}자 저장'); break
